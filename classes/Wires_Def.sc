Wires_Def : SynthDef {
	...
	*randDef {|rate, typeWeights|
		var weights;
		var res;

		weights = defs[rate].collect {|def| def.weight * typeWeights[def.type]}.normalizeSum;
		res = defs[rate][weights.windex];
		if (res.notNil) {^res}
		{
			"WARNING: no Wires_Def of type %".format(rate).postln;
			^defaultDefs[rate];
		};
	}
